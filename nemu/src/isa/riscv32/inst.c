...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (src1 >= src2) ? s->pc + imm : s->pc + 4); //pass add

  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (src1 < src2) ? s->pc + imm : s->pc + 4);  //pass bubble-sort pascal sum 
