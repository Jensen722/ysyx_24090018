...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc= s->pc + imm; IFDEF(CONFIG_FTRACE, ftrace(s->pc, s->dnpc, imm, R(rd))));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1; IFDEF(CONFIG_TRACED, ftrace(s->pc, s->dnpc, imm, R(rd))));
  ...
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (src2==0) ? 4294967295 : src1 / src2 ); 
  
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, word_t lbu_data = Mr(src1 + imm, 1);int signed_bit = lbu_data >> 7; R(rd) = (signed_bit) ? (lbu_data | 0xFFFFFF00) : lbu_data );
  ...
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10)); IFDEF(CONFIG_FTRACE, cleanup_ftrace())); // R(10) is $a0
